...
echo [1/9] Building web (Next.js) with API URL: %PRODUCTION_API_URL%
cd "%ROOT%\web"
...
echo [2/9] Preparing standalone web output...
...
echo [3/9] Copying API (excluding .venv, __pycache__, .env)...
...
echo [4/9] Copying root requirements.txt (shared venv for api + local_agent)...
copy /Y "%ROOT%\requirements.txt" "%STAGING%\devbloom\requirements.txt" >nul

echo [5/9] Copying games/ (required by API: manifest + pocket_voyager)...
...
echo [6/9] Creating archive %ZIPNAME%...
cd "%STAGING%"
tar -a -c -f "%ROOT%\deploy\%ZIPNAME%" devbloom
cd "%ROOT%"

echo [7/9] Uploading to s3://%S3_BUCKET%/%S3_PREFIX%/...
